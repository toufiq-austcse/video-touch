...
export class TranscodingService {
  ...
  async transcodeVideoByResolution(videoId: string, height: number, weidth: number) {
    let result = null;
    try {
      let inputFilePath = getLocalVideoMp4Path(videoId);
      let outputFolderPath = getLocalResolutionPath(videoId, height);

      result = await this.transcodeVideo(inputFilePath, outputFolderPath, height, weidth);
    } catch (e: any) {
      throw new Error(e);
    }
