...
export default function VideoDetailsPage() {
  ...
  const masterPlaylistUrl = videoDetails.latest_status === VIDEO_STATUS.READY ? `https://s3.ap-southeast-1.amazonaws.com/cdn.10minuteschool.com/${videoDetails._id}/master.m3u8` : null;
    ...
    <div className={'flex space-x-4'}>
      <div className={'flex flex-col space-y-4 p-8 border-2 rounded w-6/12'}>
        ...
        <div className={'border-2 min-h-[500px]'}>
          {masterPlaylistUrl ? <VideoPlayer
              source={`${masterPlaylistUrl}`} /> :
            <div className={'flex justify-center items-center h-[500px] bg-black text-white'}>Video is not ready
              yet</div>}
        </div>
        <div>
          <div>
            <Data label={'Tags'} value={videoDetails.tags.length > 0 ? videoDetails.tags : 'No Tags found' as any} />
            <Data label={'Description'} value={videoDetails.description ?? 'No description found'} />
          </div>
        </div>
      </div>
      <div className={'border-2 rounded space-y-4 p-8 w-6/12 h-3/6'}>
        <div>
          ...
          <Data label={'Created At'} value={new Date(videoDetails.created_at).toDateString()} />
          ...
          <div className={'flex justify-between'}>
            {videoDetails.status_details.map((status, index) => {
              if (index === videoDetails.status_details.length - 1) {
                return <Step key={index} title={status.status} isFinal={true} />;
              }
