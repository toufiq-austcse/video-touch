    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-wrap gap-1">
          {tags.map((tag, index) => {
            return (<Badge className="text-sm" variant="secondary" key={index}>{tag}<X className="size-4"
                                                                                       onClick={onBadgeDeleteClick(tag)} />
            </Badge>);
          })}
        </div>
      ...
      <div className="flex flex-row-reverse gap-2">
        <Button size="sm" onClick={onSaveChangeButtonClick}>Save Changes</Button>
        <Button size="sm" onClick={() => setEnableEditDetails(false)}>Close</Button>
      </div>
    </div>
        ...
        value={
          tags.length > 0
            ? (<div className="flex gap-1">
              {
                tags.map((tag, index) => {
                  return (<Badge className="text-sm" variant="secondary" key={index}>{tag}</Badge>);
                })
              }
            </div>)
